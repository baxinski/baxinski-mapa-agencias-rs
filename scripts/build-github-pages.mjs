...
const logoData = (await readFile(resolve(root, "public/branding/mapa-agencias-logo.png"))).toString("base64");
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:Arial,Helvetica,sans-serif}a{color:inherit;text-decoration:none}
    ...
    .brand{display:flex;align-items:center;gap:12px}.brand-logo{display:block;width:205px;height:76px;object-fit:contain;object-position:left center}.header-links{display:flex;align-items:center;gap:20px;font-size:12px;font-weight:800}.header-links a:last-child{padding:11px 14px;background:var(--ink);color:#fff}
...
<body>
  <header class="header"><a class="brand" href="./" aria-label="Mapa de Agências RS — início"><img class="brand-logo" src="data:image/png;base64,${logoData}" alt="André Baxinski — Mapa de Agências" width="977" height="408"></a><nav class="header-links"><a href="#diretorio">Diretório</a><a href="https://mapa-intercambio-rs.baxinski.chatgpt.site/login">Acesso da equipe ↗</a></nav></header>
  <main>
