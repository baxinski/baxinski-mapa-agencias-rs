...
export default function Dashboard() {
  ...
  return <div className="crm-layout">
    <aside className="crm-sidebar"><div className="crm-sidebar-brand"><span className="brand-mark">RS</span><div><strong>Mapa de Agências</strong><small>Operação comercial</small></div></div><nav aria-label="Menu comercial"><span className="crm-nav-label">Workspace</span><Link className="crm-nav-active" href="/dashboard">Dashboard</Link><Link href="/mapa">Mapa regional</Link><Link href="/agencias">Agências</Link><Link href="/follow-ups">Follow-ups</Link><Link href="/relatorios">Relatórios</Link><span className="crm-nav-label">Gestão</span><Link href="/agencias?status=Oportunidade%20qualificada">Oportunidades</Link><Link href="/modelos">Modelos de abordagem</Link><Link href="/leads">Leads</Link><Link href="/usuarios">Usuários e permissões</Link><Link href="/importar">Importar base</Link><Link href="/planos">Planos</Link><Link href="/admin">Painel administrativo</Link></nav><div className="crm-sidebar-note"><span>Diretório completo</span><strong>{metrics.totalAgencies.toLocaleString("pt-BR")}</strong><small>{metrics.exchangeAgencies} intercâmbio · {metrics.tourismAgencies.toLocaleString("pt-BR")} turismo</small></div></aside>
    <section className="crm-content"><header className="crm-heading"><div><span className="eyebrow">Inteligência comercial · RS</span><h1>Bom dia, equipe.</h1><p>O diretório reúne intercâmbio e turismo; o funil e os contatos são acompanhados nas fichas de intercâmbio.</p></div><Link className="button primary" href="/mapa">Explorar mapa <span>↗</span></Link></header>
      ...
      <div className="metric-grid"><Metric label="Registros no diretório" value={metrics.totalAgencies.toLocaleString("pt-BR")} hint={`${metrics.exchangeAgencies} intercâmbio · ${metrics.tourismAgencies.toLocaleString("pt-BR")} turismo regular`} /><Metric label="Oportunidades em andamento" value={metrics.opportunities} hint={`${metrics.proposals} propostas · ${metrics.negotiations} negociações`} tone="good" /><Metric label="Follow-ups atrasados" value={metrics.overdueFollowUps} hint={`${metrics.todayFollowUps} previstos para hoje`} tone={metrics.overdueFollowUps ? "alert" : "normal"} /><Metric label="Taxa de conversão" value={`${metrics.conversionRate}%`} hint={`${metrics.clients} clientes conquistados no CRM`} tone="good" /><Metric label="Contatos · últimos 7 dias" value={metrics.contactsLast7Days} hint={`${metrics.contacted} agências de intercâmbio contatadas`} /><Metric label="Valor potencial do funil" value={money(metrics.pipelineValue)} hint="Soma das oportunidades de intercâmbio" /></div>
      <div className="dashboard-grid"><BarChart title="Agências por cidade" items={charts.byCity} /><BarChart title="Agências por região" items={charts.byRegion} tone="red" /><BarChart title="Status comercial · intercâmbio" items={charts.byStatus} tone="green" /><BarChart title="Contatos por período" items={charts.contactsByDay} /></div>
