...
import { accompanimentPriorities, accompanimentStatuses, commercialStatuses, type Agency } from "@/lib/types";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.accompanimentStatus && !accompanimentStatuses.includes(body.accompanimentStatus)) return Response.json({ error: "Status de acompanhamento inválido." }, { status: 400 });
  if (body.accompanimentPriority && !accompanimentPriorities.includes(body.accompanimentPriority)) return Response.json({ error: "Prioridade inválida." }, { status: 400 });
  ...
  if (body.accompanimentStatus && body.accompanimentStatus !== current.accompanimentStatus) {
    await addStatusHistory({ id: crypto.randomUUID(), agencyId: id, previousStatus: current.accompanimentStatus ?? "Não analisada", newStatus: body.accompanimentStatus, userEmail: user?.email ?? null, note: typeof body.notes === "string" ? body.notes : null, changedAt: new Date().toISOString() });
  }
